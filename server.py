...
class handler:
    def __init__(self, conn:socket.socket) -> None:
        ...
        if self.request.split(' ')[1]!='/':
            self.lookupTable[self.request.split(' ')[1]](self.request.split(' ')[1], self.h)
        else:
    ...
    def servefile(self, filename:str, *overflow):
        try:ftype=self.filetypes[filename.split('.')[-1]]
        except:ftype=b'text/html'
        try:
            if filename=='/':
